 ...
 */
public class ChatSoundRecordPressedView extends View implements View.OnTouchListener {
    ...
    //是否允许弹出对话框
    private boolean isAllowDialogShow = false;

    public ChatSoundRecordPressedView(Context context) {
        super(context);
        init();
    }

    public ChatSoundRecordPressedView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ChatSoundRecordPressedView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (isPressed) {
                    isPressed = false;
                    postInvalidate();
                    if (callback != null && isAllowDialogShow) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (isPressed && callback != null && isAllowDialogShow) {
                    int upY = (int) event.getRawY();
                    if (pressedY - upY < getHeight()) {
                        if (isOutSize) {
                            isOutSize = false;
    ...
    public void setIsShowDialog(boolean isShowDialog) {
        isAllowDialogShow = isShowDialog;
    }
    ...
    private void initSoundVolumeDlg() {
        ...
        Objects.requireNonNull(soundVolumeDialog.getWindow()).setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN,
                WindowManager.LayoutParams.FLAG_FULLSCREEN);
        soundVolumeDialog.setContentView(R.layout.layout_chat_sound_volume_dialog);
        soundVolumeDialog.setCancelable(false);
